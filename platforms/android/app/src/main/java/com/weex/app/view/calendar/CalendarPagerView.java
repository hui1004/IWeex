...
import static com.appset.weex.costomView.calendar.MaterialCalendarView.SHOW_DEFAULTS;
import static com.appset.weex.costomView.calendar.MaterialCalendarView.showOtherMonths;
...
//            WeekDayView weekDayView = new WeekDayView(getContext(), CalendarUtils.getDayOfWeek
//                    (calendar));
//            weekDayView.setBackgroundColor(Color.parseColor("#b7c9cb"));
//            weekDayViews.add(weekDayView);
//            ViewGroup.LayoutParams layoutParams = new ViewGroup.LayoutParams(20,20);
//            weekDayView.setLayoutParams(layoutParams);
//            addView(weekDayView);
//            calendar.add(DATE, 1);
//        }
    }
    ...
    protected void addDayView(Collection<DayView> dayViews, Calendar calendar) {
        Log.e("calendar", CalendarDay.from(calendar) + "");
        ...
        for (DayView dayView : dayViews) {
            ...
            if (dates.contains(day)) {
                if (dates instanceof ArrayList) {
                    ArrayList<CalendarDay> arrayList = (ArrayList<CalendarDay>) dates;
                    CalendarDay find = arrayList.get(arrayList.indexOf(day));
                    dayView.setExactlyDaySelected(find.isExacyltSameDay());

                }
                dayView.setChecked(true);
            } else {
                dayView.setChecked(false);
                dayView.setExactlyDaySelected(false);
            }
        }
        postInvalidate();
    }
    ...
    protected void updateUi() {
        for (DayView dayView : dayViews) {
            CalendarDay day = dayView.getDate();
            boolean isInRange = day.isInRange(minDate, maxDate);
    ...
    protected void onMeasure(final int widthMeasureSpec, final int heightMeasureSpec) {
        ...
        if (specHeightMode == MeasureSpec.UNSPECIFIED || specWidthMode == MeasureSpec.UNSPECIFIED) {
            throw new IllegalStateException("CalendarPagerView should never be left to decide " +
                    "it's size");
        }
